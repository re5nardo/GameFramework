...
#include "../../../FBSFiles/WorldSnapShotToC_generated.h"
...
const char* WorldSnapShotToC::Serialize(int* pLength)
{
	...
	list<Offset<FBS::EntityState>> listEntityState;
	for (map<int, IEntity*>::iterator it = m_mapEntity.begin(); it != m_mapEntity.end(); ++it)
	{
		...
		FBS::Data::Vector3 pos(pEntity->GetPosition().x(), pEntity->GetPosition().y(), pEntity->GetPosition().z());
		FBS::Data::Vector3 rot(pEntity->GetRotation().x(), pEntity->GetRotation().y(), pEntity->GetRotation().z());
		...
		FBS::EntityStateBuilder entityState_builder(m_Builder);
	...
	vector<Offset<FBS::EntityState>> vecOffsetEntityState;
	for (list<Offset<FBS::EntityState>>::iterator it = listEntityState.begin(); it != listEntityState.end(); ++it)
	{
		vecOffsetEntityState.push_back(*it);
	}
	auto entityStates = m_Builder.CreateVector(vecOffsetEntityState);

	FBS::WorldSnapShotToCBuilder data_builder(m_Builder);
...
{
	auto data = flatbuffers::GetRoot<FBS::WorldSnapShotToC>((const void*)pChar);
